...
const UNIQUE_CONSTRAINTS = [
  ...
  [
    "payroll_periods",
    "payroll_periods_period_from_period_to_unique",
...
exports.up = async function (knex) {
  ...
  await knex.schema.createTable("company_subscriptions", (table) => {
    ...
    table
      .enu("status", ["trial", "active", "overdue", "suspended"])
      .notNullable()
      .defaultTo("trial");
    table.timestamps(true, true);
  });
  ...
  for (const tableName of TENANT_TABLES) {
    ...
    await knex.raw(
      `ALTER TABLE ?? ALTER COLUMN company_id SET DEFAULT (NULLIF(current_setting('app.current_company_id', true), '')::integer)`,
      [tableName],
    );
    await knex.raw(`CREATE INDEX ?? ON ?? (company_id)`, [
      `${tableName}_company_id_index`,
      tableName,
    ]);
  }
  ...
  await knex.raw(
    `CREATE UNIQUE INDEX users_one_super_admin_per_company ON users (company_id) WHERE role = 'super_admin' AND is_deleted = false`,
  );
