...
const run = async () => {
  ...
  const [other] = await db
    .root("companies")
    .insert({ name: "HTTP Smoke Company", slug: "http-smoke-company" })
    .returning("*");
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          own_company_status: own.status,
          cross_company_status: cross.status,
          isolated: own.status === 200 && cross.status === 401,
        },
        null,
        2,
      ),
