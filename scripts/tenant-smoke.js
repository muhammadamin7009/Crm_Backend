...
const run = async () => {
  ...
  const existingUser = await db
    .root("users")
    .where({ company_id: zerr.id, is_deleted: false })
    .orderBy("id")
    .first();
  let result;
  ...
  try {
    await db.root.transaction(async (trx) => {
      const [perfect] = await trx("companies")
        .insert({ name: "Perfect Shoes Smoke", slug: "perfectshoes-smoke" })
        .returning("*");
      ...
      const sameUsername = await trx("users")
        .where({ username: existingUser.username })
        .count({ count: "*" })
        .first();
      result = {
        duplicate_username_allowed: true,
        perfect_visible_users: Number(perfectUsers.count),
        zerr_visible_users: Number(zerrUsers.count),
        zerr_same_username_count: Number(sameUsername.count),
      };
