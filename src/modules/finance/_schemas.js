...
exports.payrollLineUpdate = {
  params,
  body: Joi.object({
    daily_earnings: Joi.number().min(0),
    bonus: Joi.number().min(0),
    advance_deduction: Joi.number().min(0),
    other_deduction: Joi.number().min(0),
    cash_amount: Joi.number().min(0),
    note: Joi.string().allow(null, ""),
  }).min(1),
};
...
exports.accountCreate = {
  body: Joi.object({
    name: Joi.string().trim().max(100).required(),
    account_type: Joi.string().valid("cash", "card", "bank").required(),
