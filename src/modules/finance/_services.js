...
const listPayroll = async ({ limit = 50, offset = 0 }) => {
  ...
  const [rows, count] = await Promise.all([
    query.clone().select("pp.*").select(db.raw("(SELECT COALESCE(SUM(total_earned),0) FROM payroll_lines WHERE period_id=pp.id) AS total_earned"), db.raw("(SELECT COALESCE(SUM(cash_amount),0) FROM payroll_lines WHERE period_id=pp.id) AS cash_amount")).orderBy("period_to", "desc").limit(Number(limit)).offset(Number(offset)),
    query.clone().count({ count: "id" }).first(),
  ]);
  return { payroll_periods: rows, pageInfo: { total: n(count.count), limit: n(limit), offset: n(offset) } };
};
...
const showPayroll = async (id) => {
  ...
  const lines = await db("payroll_lines as pl").join("employee_profiles as ep", "ep.id", "pl.employee_id").join("users as u", "u.id", "ep.user_id").leftJoin("positions as p", "p.id", "ep.position_id").where("pl.period_id", id).select("pl.*", "ep.user_id", "u.first_name", "u.last_name", "u.username", "p.name as position_name").orderBy("u.first_name");
...
const createPayroll = async (body, actor) => {
  ...
  const periodId = await db.transaction(async (trx) => {
    ...
    for (const employee of employees) {
      ...
      const output = await trx("worker_outputs").where({ worker_id: employee.user_id, is_deleted: false }).whereBetween("worked_at", [body.period_from, body.period_to]).sum({ amount: "total_amount" }).countDistinct({ days: "worked_at" }).first();
      const sales = await trx("client_sales").where({ created_by: employee.user_id, is_deleted: false }).whereBetween("sold_at", [body.period_from, body.period_to]).sum({ amount: "total_amount" }).first();
      const type = agreement.payment_type;
      const piece = ["piece_rate", "mixed"].includes(type) ? n(output.amount) : 0;
      const fixed = ["fixed_salary", "mixed"].includes(type) ? (agreement.payment_period === "monthly" ? n(agreement.fixed_amount) * periodDays / 30 : n(agreement.fixed_amount)) : 0;
      const daily = type === "daily_rate" ? n(output.days) * n(agreement.daily_rate) : 0;
      const commission = type === "commission" ? n(sales.amount) * n(agreement.commission_percent) / 100 : 0;
      const total = piece + fixed + daily + commission;
      await trx("payroll_lines").insert({ period_id: period.id, employee_id: employee.id, piece_earnings: piece, fixed_earnings: fixed, daily_earnings: daily, commission_earnings: commission, total_earned: total, cash_amount: total });
    }
    return period.id;
  });
...
const closePayroll = async (id, actor) => {
  const period = await db("payroll_periods").where({ id }).first();
  if (!period) throw new NotFoundError("Haftalik ish haqi davri topilmadi");
  if (period.status === "closed") throw new BadRequestError("Haftalik ish haqi hisobi allaqachon yopilgan");
  await db.transaction(async (trx) => {
    ...
    await trx("payroll_periods").where({ id }).update({ status: "closed", closed_by: actor.id, closed_at: trx.fn.now(), updated_at: trx.fn.now() });
  });
  return showPayroll(id);
};

const listCategories = async () => ({ expense_categories: await db("expense_categories").where({ is_deleted: false }).orderBy("name") });
const createCategory = async (body) => ({ expense_category: (await db("expense_categories").insert({ name: body.name, description: clean(body.description) }).returning("*"))[0] });
const listExpenses = async (filters) => {
  const query = range(db("expenses as e").join("expense_categories as ec", "ec.id", "e.category_id").leftJoin("financial_accounts as fa", "fa.id", "e.account_id").where("e.is_deleted", false), "e.spent_at", filters);
  const rows = await query.clone().select("e.*", "ec.name as category_name", "fa.name as account_name").orderBy("e.spent_at", "desc").limit(n(filters.limit || 50)).offset(n(filters.offset));
...
const listReturns = async (filters) => {
  ...
  return { client_returns: await query.select("cr.*", "p.name as product_name", db.raw("CONCAT(u.first_name,' ',u.last_name) as client_name")).orderBy("cr.returned_at", "desc").limit(n(filters.limit || 50)).offset(n(filters.offset)) };
};
const createReturn = async (body, actor) => {
  const sale = await db("client_sales").where({ id: body.client_sale_id, is_deleted: false }).first();
  if (!sale) throw new NotFoundError("Savdo topilmadi");
  const returned = await db("client_returns").where({ client_sale_id: sale.id, is_deleted: false }).sum({ quantity: "quantity" }).first();
  if (n(returned.quantity) + n(body.quantity) > n(sale.quantity)) throw new BadRequestError(`Qaytarish miqdori qolgan ${n(sale.quantity) - n(returned.quantity)} dan oshmasin`);
  const [row] = await db("client_returns").insert({ client_sale_id: sale.id, client_id: sale.client_id, product_id: sale.product_id, quantity: body.quantity, amount: n(body.quantity) * n(sale.unit_price), returned_at: body.returned_at || db.fn.now(), reason: clean(body.reason), created_by: actor.id }).returning("*");
  return { client_return: row };
};

const profitLoss = async (filters) => {
  const sum = async (table, column, dateColumn, extra = {}) => n((await range(db(table).where(extra), dateColumn, filters).sum({ value: column }).first()).value);
  ...
  const payrollQuery = range(db("payroll_lines as pl").join("payroll_periods as pp", "pp.id", "pl.period_id"), "pp.payment_date", filters);
  const payrollSum = n((await payrollQuery.sum({ value: "pl.total_earned" }).first()).value);
  const netRevenue = sales - returns;
  return { report: { sales, returns, net_revenue: netRevenue, material_costs: materials, payroll_costs: payrollSum, other_expenses: expenses, operational_result: netRevenue - materials - payrollSum - expenses } };
};

module.exports = { listPayroll, showPayroll, createPayroll, updatePayrollLine, closePayroll, listCategories, createCategory, listExpenses, createExpense, listAccounts, createAccount, listTransactions, createTransaction, listReturns, createReturn, profitLoss };
