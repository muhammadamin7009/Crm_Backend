...
const deviceName = (userAgent = "") => {
  const browser = /Edg/i.test(userAgent) ? "Edge" : /Firefox/i.test(userAgent) ? "Firefox" : /Chrome/i.test(userAgent) ? "Chrome" : /Safari/i.test(userAgent) ? "Safari" : "Brauzer";
  const os = /Windows/i.test(userAgent) ? "Windows" : /Android/i.test(userAgent) ? "Android" : /iPhone|iPad/i.test(userAgent) ? "iOS" : /Mac OS/i.test(userAgent) ? "macOS" : /Linux/i.test(userAgent) ? "Linux" : "Noma'lum qurilma";
  return `${browser} / ${os}`;
};

const publicUser = (existing, company) => ({
  id: existing.id,
...
const issueSession = async (existing, company, meta) => {
  ...
  const token = jwt.sign(
    { id: existing.id, role: existing.role, company_id: existing.company_id, company_slug: company.slug, jti, session_id: sessionId },
    config.jwt.secret,
    { expiresIn: "2d" },
  );

  if (!knownDevice && existing.role === "super_admin" && existing.phone) {
    await sendSms({ phone: existing.phone, message: `ZERR CRM: profilingizga yangi qurilmadan kirildi: ${deviceName(meta.user_agent)}. Agar bu siz bo'lmasangiz, boshqa qurilmalardan chiqishni bosing.` });
  }

  return { token, user: publicUser(existing, company) };
};

const findUser = (username) => db("users")
  .where({ username, is_deleted: false })
  .select("id", "username", "password", "user_image", "role", "first_name", "last_name", "phone", "company_id")
  .first();
...
const login = async ({ username, password, device_id }, company, metadata = {}) => {
  ...
  const recent = await db("auth_challenges")
    ...
    .count({ count: "id" })
    .first();
  if (Number(recent.count) >= 3) throw new BadRequestError("Kod juda ko'p so'raldi. 10 daqiqadan keyin urinib ko'ring");
  ...
  await sendSms({ phone: existing.phone, message: `ZERR CRM tasdiqlash kodi: ${code}. Kod 5 daqiqa amal qiladi.` });

  return { mfa_required: true, challenge_id: challengeId, masked_phone: maskPhone(existing.phone), expires_in: 300 };
};

login.verify = async ({ challenge_id, code }, company) => {
  const challenge = await db("auth_challenges").where({ id: challenge_id }).first();
  if (!challenge || challenge.consumed_at || new Date(challenge.expires_at) <= new Date()) throw new UnauthorizedError("Tasdiqlash kodi eskirgan");
  ...
  return issueSession(existing, company, { device_id: challenge.device_id, user_agent: challenge.user_agent, ip_address: challenge.ip_address });
};
