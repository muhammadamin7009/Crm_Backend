...
const createWorkerAdvance = async (req, res, next) => {
  try {
    httpValidator({ body: req.body }, schemas.createWorkerAdvanceSchema);
    res.status(201).json(await createService(req.body, req.user));
  } catch (error) {
    next(error);
  }
};
const getWorkerAdvances = async (req, res, next) => {
  try {
    httpValidator({ query: req.query }, schemas.listWorkerAdvancesSchema);
    res.json(await listService(req.query, req.user));
  } catch (error) {
    next(error);
  }
};
const getWorkerAdvanceBalance = async (req, res, next) => {
  try {
    httpValidator({ query: req.query }, schemas.workerAdvanceBalanceSchema);
    res.json(await balanceService(req.query, req.user));
  } catch (error) {
    next(error);
  }
};
const getWorkerAdvance = async (req, res, next) => {
  try {
...
const removeWorkerAdvance = async (req, res, next) => {
  try {
    httpValidator({ params: req.params }, schemas.deleteWorkerAdvanceSchema);
    res.json(await deleteService({ id: Number(req.params.id) }));
  } catch (error) {
    next(error);
  }
};
